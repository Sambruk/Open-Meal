$(function () {
  var openMealDataProviderURL =
    "https://raw.githubusercontent.com/Sambruk/Open-Meal/main/dataproviders.json";
  ...
  var dataProiderXHR = $.getJSON(openMealDataProviderURL, function (response) {
    ...
    $("#openMealWizard #openMealWizard .alert")
      .html("<strong>Error:</strong> Could not get data providers")
      .show();
  });
  ...
  $("#openMealWizard #dataProviders").change(function () {
    ...
    var distributorXHR = $.getJSON(
      currentOption.data("baseurl") + "/openmeal/v2/distributors.json",
      function (response) {
          ...
          $("#openMealWizard #distributors").append(
            $("<option />")
              .text(
                distributor.name +
                  " (" +
                  distributor.address.addressLocality +
                  ")"
              )
      ...
      $("#openMealWizard #distributorsContainer .loadingImage").hide();
      $("#openMealWizard .alert")
        .html(
          "<strong>Error:</strong> Could not get distributors from " +
            currentOption.val()
        )
        .show();
    });
  ...
  $("#openMealWizard #distributors").change(function () {
    ...
    var openMealURL =
      currentProvider.data("baseurl") +
      "/openmeal/v2/meals.json?distributorID=" +
      currentOption.val();
    var code =
      '<div id="openMealWidget" data-language="' +
      currentOption.data("language") +
      '" data-openmealurl="' +
      openMealURL +
      '"></div>\n' +
